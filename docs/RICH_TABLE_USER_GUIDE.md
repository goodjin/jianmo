...
3. **粘贴建表**（Rich，光标在**表格外**）：从 Excel、Numbers、网页等复制后粘贴，若剪贴板能解析为矩阵（`text/html` 表格或 `text/plain` 制表分隔等），可在当前位置**插入新表**并重排内容。粘贴路径有**行 / 列 / 总格子数上限**（硬上限：**80** 行、**40** 列、**800** 格；超出会拒绝或截断并提示）。
